...
Mint.save = function() 
{
	...
	var path	= 'http://blog.theoleg.com/mint/?record&key=6d647658333031373237336f5274514c4d536f683553703431';
...
{
	return /^([^:]+):\/\/([a-z0-9]+[\._-])*(blog\.theoleg\.com)/i.test(url);
};
function Mint_SI_IO_save()
{
	var now		= new Date();
	var then	= now.getTime() + 300;
	var path	= 'http://blog.theoleg.com/mint/pepper/shauninman/outbound/click.php?'+now.getTime();
	path 		= path.replace(/^https?:/, window.location.protocol);
